...
const TrafficIncidentMarker = ({ incident }: { incident: TrafficIncident }) => (
  <Marker coordinate={incident.location}>
    <View style={styles.incidentMarker}>
      <MaterialIcons 
        name={incident.severity === "high" ? "warning" : "info"} 
        size={24} 
...
export default function NavigationApp({ navigation }: { navigation: any }) {
  ...
  const speakNavigationInstruction = useCallback((text: string) => {
    if (ttsAvailable) {
      Tts.speak(text);
    }
  }, [ttsAvailable]);

  const safeTtsSpeak = (text: string) => {
    try {
      if (Tts) {
        Tts.speak(text);
      }
  ...
  // Usage:
  safeTtsSpeak("Starting navigation");
  ...
  const handleVoiceNavigation = useCallback((instruction: string) => {
    if (ttsInitialized) {
  ...
  useEffect(() => {
    ...
    // Voice navigation
    if (voiceEnabled && selectedRoute.instructions) {
      const nextInstruction = selectedRoute.instructions[0];
      if (nextInstruction) {
        voiceNavTimeout.current = setTimeout(() => {
          Tts.speak(nextInstruction);
        }, VOICE_NAV_DELAY);
      }
    }
  ...
  const startNavigation = useCallback(() => {
    ...
    Tts.speak("Starting navigation to " + (destination?.address || "your destination"));
